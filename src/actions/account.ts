...
export async function deleteAccountAction(): Promise<ActionResult> {
  ...
  // (Phase-2 note). `transactions.staff_id` / `audit_log.actor_id` are RESTRICT
  // FKs kept to preserve the audit trail; they are SET NULL here. The user's OWN
  ...
  //
  // This step is LOAD-BEARING, not best-effort: for a staff/admin account those
  // RESTRICT references exist, and if they aren't NULLed the auth-user delete is
  // FK-blocked. So if the anonymize fails we STOP and surface an error rather than
  // proceed to a delete that would fail with a confusing message. (For a normal
  // customer these updates touch 0 rows and succeed trivially.)
  const { error: txAnonError } = await service
    .from("transactions")
    .update({ staff_id: null })
    .eq("staff_id", user.id);
  const { error: auditAnonError } = await service
    .from("audit_log")
    .update({ actor_id: null })
    .eq("actor_id", user.id);
  if (txAnonError || auditAnonError) {
    return {
      ok: false,
